...
export const AGENT_OUTPUT_CONTRACTS: Partial<Record<AgentKey, string>> = {
  ...
  planning: [
    ...
    "        • Carrossel: \"slides\": array com NO MÁXIMO 4 telas (JAMAIS mais que 4). Um bom carrossel é um ROTEIRO, não uma legenda picada: as telas contam UMA história conectada que leva o leitor até a última — não frases soltas. Fórmula mental obrigatória: Curiosidade → Contexto → Consequência → Solução. FUNÇÃO DE CADA TELA: Tela 1 = gerar identificação, curiosidade ou tensão (apresenta a situação e faz deslizar); Tela 2 = aprofundar o problema (explica POR QUE acontece); Tela 3 = mostrar consequências, impactos e implicações reais (o que está em jogo); Tela 4 = solução/conclusão + CTA que fecha o raciocínio. Se o tema pedir 3 telas, funda as etapas mantendo essa progressão.",
    "          Cada slide é { title, text }: `title` = rótulo curto (até 6 palavras) que nomeia a tela; `text` = copy DESENVOLVIDA de 3 a 6 frases (aprox. 45 a 90 palavras) que constrói raciocínio — contexto + explicação (por que acontece) + consequência + exemplo/situação real + uma ponte que puxa a próxima tela. É PROIBIDO frase de efeito solta (ex.: \"A peça parece igual\", \"Escolher errado custa caro\"): sempre desenvolva o contexto por trás. Cada tela deve ENTREGAR VALOR e ensinar algo novo — ao terminar o slide, o leitor sente vontade de ver o próximo.",
    "          Antes de finalizar CADA tela, valide (se algum \"não\", REESCREVA): o leitor entendeu o problema? compreendeu a importância? há informação NOVA (não é a mesma ideia com outras palavras)? a transição para a próxima tela faz sentido? O conteúdo deve ser compreensível para quem tem pouco conhecimento técnico e, ao mesmo tempo, transmitir autoridade para quem já conhece o mercado. Profundidade acima de quantidade: o objetivo não é preencher telas, é fazer o usuário chegar até a última.",
