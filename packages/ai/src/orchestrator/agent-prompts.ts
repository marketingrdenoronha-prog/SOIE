...
export const AGENT_OUTPUT_CONTRACTS: Partial<Record<AgentKey, string>> = {
  ...
  planning: [
    ...
    "        • Vídeo/Motion: \"estimatedDuration\" (entre 30s e 1min20s — nunca menos) e \"sections\": array com as 5 partes na ordem { label, text }: Gancho, Conexão, Desenvolvimento, Virada, CTA. A copy tem que ser SUFICIENTE PARA GRAVAR: mínimo ~130 palavras no total (roteiro de ~40s+), cada seção com 2–4 frases. NUNCA gere roteiros curtos de uma linha.",
    "        • Carrossel: \"slides\": array de { title, text } (6 a 8 slides). Slide 1 = headline extremamente forte; slides do meio = desenvolvimento COMPLETO (não resuma, contexto suficiente); último slide = conclusão + CTA.",
    "        • Estático: \"static\": { headline, subheadline, body (texto principal desenvolvido, com profundidade — 3+ frases), cta, designNotes }.",
    "CONTEXTO (obrigatório e assertivo): use ATIVAMENTE context.dossier, context.onboarding, context.audienceSignals (dores e desejos REAIS do público), context.brandVoice e context.business. Cite dores/desejos concretos do cliente e a proposta de valor — proibido conteúdo genérico que serviria para qualquer marca.",
    "QUALIDADE (valide ANTES de finalizar; se qualquer resposta for negativa, REESCREVA): a quantidade por formato bate com formatCounts? há contexto suficiente do dossiê/onboarding? há desenvolvimento? o roteiro de vídeo tem ~40s+ (≥130 palavras)? há começo, meio e fim? há CTA contextualizada? um Designer/Filmaker conseguiria produzir só lendo isto?",
