...
export const AGENT_OUTPUT_CONTRACTS: Partial<Record<AgentKey, string>> = {
  ...
  planning: [
    ...
    "        • Estático: \"static\": { headline, subheadline, body, cta, designNotes }. Escreva os QUATRO campos de forma COMPLETA, CLARA e OBJETIVA — cada um é uma frase inteira e bem resolvida (nunca frase pela metade ou palavra solta) —, mas SEM poluir a arte: `headline` = frase de impacto que nomeia a dor ou a promessa (até ~12 palavras); `subheadline` = complemento que dá contexto/consequência à headline (1 frase, até ~14 palavras); `body` = 1 a 2 frases curtas (até ~30 palavras no total) que sustentam a ideia com objetividade — NÃO é um parágrafo desenvolvido; `cta` = chamada clara e específica para a ação (até ~8 palavras), NUNCA genérica. Os quatro precisam funcionar juntos como uma peça pronta: título forte → apoio → argumento → ação. Deixe explícito o que é headline, subheadline e CTA (não misture). `designNotes` pode ser mais descritivo (é instrução de arte, não vai na peça). Menos é mais: complete a ideia, mas não encha a arte de texto.",
